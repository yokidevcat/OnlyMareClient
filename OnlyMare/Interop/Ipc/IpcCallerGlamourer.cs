...
public sealed class IpcCallerGlamourer : DisposableMediatorSubscriberBase, IIpcCaller
{
    ...
    public IpcCallerGlamourer(ILogger<IpcCallerGlamourer> logger, IDalamudPluginInterface pi, DalamudUtilService dalamudUtil, OnlyMareMediator onlymareMediator,
        RedrawManager redrawManager) : base(logger, onlymareMediator)
    {
    ...
    public void CheckAPI()
    {
        ...
        finally
        {
            if (!apiAvailable && !_shownGlamourerUnavailable)
            {
                ...
                _onlymareMediator.Publish(new NotificationMessage("Glamourer inactive", "Your Glamourer installation is not active or out of date. Update Glamourer to continue to use OnlyMare. If you just updated Glamourer, ignore this message.",
        ...
        try
        {
            ...
            await _redrawManager.PenumbraRedrawInternalAsync(logger, handler, applicationId, (chara) =>
            {
                try
                {
                    ...
                    _onlymareMediator.Publish(new PenumbraRedrawCharacterMessage(chara));
                }
                catch (Exception ex)
                {
    ...
    {
        _onlymareMediator.Publish(new GlamourerChangedMessage(address));
    }
}
